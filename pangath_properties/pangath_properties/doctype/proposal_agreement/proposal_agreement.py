...
class ProposalAgreement(Document):
...
def create_tenancy_contract(args):
	...
	to_doc = frappe.get_doc('Proposal Agreement', args.get("name"))
	...
	tc.yearly_rent = frappe.db.get_value("Unit", {"name": to_doc.unit}, "rent")
	tc.cost_center = frappe.db.get_value("Cost Center", {"company": to_doc.company}, "name")
	tc.proposal_agreement = args.get("name")
...
def cancel_tenant_onboarding(doc, values):
	if isinstance(doc, str):
		doc = json.loads(doc)
		doc = frappe._dict(doc)
	to_doc = frappe.get_doc('Proposal Agreement', doc.name)
...
def auto_create_payment_entry(docname):
	doc = frappe.get_doc("Proposal Agreement", docname)
