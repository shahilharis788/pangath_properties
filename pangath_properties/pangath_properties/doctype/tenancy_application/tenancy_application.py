...
def get_tax_rate(tax):
    rate = frappe.db.get_all(
        ...
        filters={"parent": tax},
        fields=["sum(tax_rate) as total_tax"]
    )

    if rate and rate[0].total_tax:
        return rate[0].total_tax
    else:
        return 0