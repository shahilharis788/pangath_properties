...
def populate_payment_schedule(self):
    ...
    freq = 0
    req_part = []
    sp = len([row for row in self.type_of_charges if row.custom_is_scheduled_payment])
    if self.schedule_payments and self.schedule_payments[-1].number_of_period > 1 and self.schedule_payments[-1].payment_frequency != '1 Payment' and sp <=1:
