...
from frappe.utils.data import format_date, get_datetime_str
...
def get_currency1(filters):
	...
	:return: str - Currency
	"""
	company = filters.company if filters.company else [get_default_company()]
	currency_list = frappe.utils.unique(frappe.db.get_all("Company", filters={"name": ["in", filters.company],}, pluck="default_currency"))
	company_currency = currency_list[0] if currency_list else None
	...
	if not report_date:
		...
		report_date = format_date(get_datetime_str(fiscal_year_to_date), "dd-MM-yyyy")
...
def get_gl_entries(filters, accounting_dimensions):
	currency_map = get_currency1(filters)
	...
	if filters.get("include_default_book_entries"):
		company_fbs = frappe.utils.unique(frappe.db.get_all("Company", filters={"name": ["in", filters.company],"default_finance_book" :["!=", ""] }, pluck="default_finance_book"))
		filters["company_fb"] = None if len(company_fbs) == 1 and not company_fbs[0] else company_fbs
	dimension_fields = ""
	if accounting_dimensions:
		dimension_fields = ", ".join(accounting_dimensions) + ","
	companies = ", ".join([frappe.db.escape(i) for i in filters.company])
	gl_entries = frappe.db.sql(
		...
		from `tabGL Entry`
		where company in ({companies}) {conditions}
		{order_by_statement}
	""".format(
...
def get_conditions(filters):
	...
	if filters.get("include_default_book_entries"):
		if filters.get("finance_book"):
			if filters.get("company_fb") and not cstr(filters.get("finance_book")) in cstr(
			...
			finance_book_cond = "finance_book IS NULL"
			if filters.company_fb:
				finance_book_cond = f"(finance_book in (%(company_fb)s, '') OR {finance_book_cond})"
			conditions.append(finance_book_cond)
	else:
		if filters.get("finance_book"):
			conditions.append("(finance_book in (%(finance_book)s, '') OR finance_book IS NULL)")
		else:
...
def get_account_type_map(company):
	account_type_map = frappe._dict(
		frappe.get_all(
			"Account", fields=["name", "account_type"], filters={"company": ["in",company]}, as_list=1
		)
	)
